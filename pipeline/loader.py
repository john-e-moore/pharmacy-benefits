...
from pipeline.config import PROJECT_ROOT, RAW_DIR, load_project_env
from pipeline.synthetic.common import setup_logging

logger = setup_logging("pipeline.loader")
...
def _load_table(conn, table_config: dict) -> int:
...
def load_all_raw_tables() -> dict:
    ...
    try:
        ...
        for table_config in RAW_TABLES:
            table_key = f"{table_config['schema']}.{table_config['table']}"
            results[table_key] = _load_table(conn, table_config)

        logger.info("All raw tables loaded successfully")
        return {"tables_loaded": len(results), "rows_by_table": results}
    finally:
        conn.close()
