...
from pipeline.config import LOG_DIR, RAW_DIR, TIMESTAMP_FILE, load_project_env
...
def validate_claims_integrity(
    ...
    for _, row in claims_df.iterrows():
        ...
        if row["claim_status"] == "rejected" and (
            row["plan_paid"] != 0 or row["member_copay"] != 0
        ):
            raise ValueError(
                f"Rejected claim {row['claim_id']} has non-zero payment amounts"
            )
